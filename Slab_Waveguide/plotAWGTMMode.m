function plotAWGTMMode(beta_field,k,ns,nf,nc,h_s,h_f,h_c)
    ...
    for i = 1:NN+1
        ...
        if (x_step(i) <= h_s);
            ...
            H_y(i) = A_s*exp(gamma_s*(x_step(i) - h_s));

        elseif (h_s <= x_step(i)) && (x_step(i) <= h_s + h_f);

            H_y(i) = A_s*(cos(kappa_f*(x_step(i) - h_s)) + gamma_s*sin(kappa_f*(x_step(i)- h_s))./kappa_f);
        ...
        elseif (h_s + h_f <= x_step(i)) && (x_step(i) <= thickness);
            ...
            %have to be matched
            H_y(i) = A_s*(cos(kappa_f*h_f) + gamma_s * sin(kappa_f*h_f)/kappa_f)*exp(-gamma_c*(x_step(i) - h_f - h_s)); 
    ...
    ylabel('TM Magnetic field field','FontSize',22);
